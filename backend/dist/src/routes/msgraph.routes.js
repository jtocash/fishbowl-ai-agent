...
router.post("/sendmail", async (req, res) => {
    try {
        ...
        const response = await msgraph_service_1.msGraphService.sendEmail('support@renewedwarehouse.com', req.body.subject, req.body.body);
        res.json(response);
    }
    catch (err) {
        console.error("Microsoft Graph error:", err.response?.data || err);
...
router.all("/webhook", async (req, res) => {
    try {
        ...
        if (!value[0] || value[0].clientState !== environment_1.config.webhooks.clientState) {
